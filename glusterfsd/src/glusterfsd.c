...
emancipate (int ret)
{
        ...
        if (process_ctx.daemon_pipe[1] != -1) {
                write (process_ctx.daemon_pipe[1], (void *) &ret, sizeof (ret));
                close (process_ctx.daemon_pipe[1]);
...
daemonize (glusterfs_vol_ctx_t *ctx)
{
        ...
        case -1:
                if (process_ctx.daemon_pipe[0] != -1) {
                        close (process_ctx.daemon_pipe[0]);
                        close (process_ctx.daemon_pipe[1]);
                }
        ...
        default:
                ...
                err = 1;
                read (process_ctx.daemon_pipe[0], (void *)&err, sizeof (err));
                _exit (err);
        }
